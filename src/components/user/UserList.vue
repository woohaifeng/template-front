<template>
  <el-container>
    <el-aside style="width: 200px;padding:3px 3px;">
      <el-card class="box-card" style="height: 99%;">
        <div slot="header" class="clearfix">
          <span>部门</span>
          <el-button style="float: right; padding: 3px 0" type="text" icon="el-icon-caret-left"></el-button>
        </div>
      ...
      <div style="height:100%;" class="data-list">
        ...
        <el-divider content-position="right" style="margin:18px 0;">
          ...
          <el-tooltip class="item" effect="dark" content="删除" placement="top-start">-->
            <el-button :disabled="multipleSelection.length==0" v-popover="'del_popover'" type="danger"
                       icon="el-icon-delete"
        ...
        <!--创建模态框-->
        <el-dialog title="创建部门" :visible.sync="flagDialogCreateVisible" center :append-to-body='true'
                   :lock-scroll="false"
                   top="8vh" width="60%" :close-on-click-modal="false" :close-on-press-escape="false"
                   :show-close="false">
          <dept-add
            :form="form"
            :ok="add"
...
<script>
  ...
  export default {
    ...
    methods: {
      loadData(search) {
        search.name = "abc";
        https.get("/dept/list", search).then((response) => {
